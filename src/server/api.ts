...
// 开小灶教程的食材行与结构化步骤校验
const tutorialIngredients=(value:unknown)=>{if(value===undefined)return undefined;if(!Array.isArray(value)||value.length<1||value.length>40)throw new ValidationError("ingredients","ingredients 必须是 1 到 40 项");return value.map((item)=>{if(!item||typeof item!=="object"||Array.isArray(item))throw new ValidationError("ingredients","ingredients 中的每一项必须是对象");const ing=item as Record<string,unknown>;const note=text(ing,"note",{max:200,allowEmpty:true});return{name:text(ing,"name",{required:true,max:120})!,amount:text(ing,"amount",{required:true,max:80})!,...(note?{note}:{})}})};
...
function resources(db:RecipeDB):Record<string,Resource>{return{
  ...
  tutorials:{list:()=>db.getTutorials(),get:id=>{const t=db.getRecipe(id);return t&&(t.source==="tutorial"||t.source==="preset")?t:null},
    create:b=>db.createRecipe({title:text(b,"title",{required:true,max:200})!,ingredients:tutorialIngredients(b.ingredients),steps:tutorialSteps(b.steps),source:"tutorial"}),
    update:(id,b)=>db.updateRecipe(id,{title:text(b,"title",{max:200}),ingredients:b.ingredients===undefined?undefined:tutorialIngredients(b.ingredients),steps:b.steps===undefined?undefined:tutorialSteps(b.steps)}),
    remove:id=>db.archiveRecipe(id)},
}}
